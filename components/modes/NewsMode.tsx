...
export function NewsMode() {
  ...
  */
      ...
      try {
        ...
        if (mounted && Array.isArray(data)) {
          ...
          const mapped: NewsItem[] = data.map((row: any) => {
            ...
            if (rawNested) {
              const nestedLat = rawNested.lat ?? rawNested.latitude ?? rawNested.Lat ?? rawNested.Latitude ?? null;
              const nestedLon = rawNested.lon ?? rawNested.longitude ?? rawNested.Lon ?? rawNested.Longitude ?? null;
            ...
            const categories =
              Array.isArray(row.category) ? row.category : row.category ? [row.category] : undefined;

              // Normalize severity to lowercase string (e.g. 'critical', 'warning', 'info', 'normal')
              const rawSeverity = (row.severity as any) ?? (row.status as any) ?? null;
              const severity = rawSeverity ? String(rawSeverity).toLowerCase() : undefined;
              ...
              category: categories,
              location,
              district: row.district ?? row.District ?? row.location_district ?? undefined,
          ...
          if (firstDistrict) {
            try {
              const dRes = await fetch(`/api/districts/${encodeURIComponent(firstDistrict)}`);
              if (dRes.ok) {
                const dJson = await dRes.json();
                // Use raw backend risk_level string as area status and keep it unchanged.
                const rawStatus = dJson.risk_level ?? dJson.riskLevel ?? dJson.risk ?? undefined;
  ...
  if (loading) {
    return (
      <EmptyState icon={Newspaper} message="Loading news..." />
    );
  }
